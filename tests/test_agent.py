...
def test_delete_requires_confirmation_then_executes_on_next_turn(monkeypatch):
    ...
    responses = [
        ...
        _fake_message(tool_calls=[_fake_tool_call("call_5b", "delete_calendar_event", delete_args)]),
...
def test_confirmed_delete_chains_into_a_followup_action_same_turn(monkeypatch):
    ...
    responses = [
        ...
        # Confirmation turn: delete executes, then the model keeps going
        # and creates the follow-up reminder in the SAME turn.
        _fake_message(tool_calls=[_fake_tool_call("call_10b", "delete_reminder", delete_args)]),
        _fake_message(
            tool_calls=[
                _fake_tool_call(
                    "call_10c",
...
def test_declining_a_pending_delete_clears_it_without_executing(monkeypatch):
    ...
    responses = [
        ...
        _fake_message(content="Okay, I won't delete it."),
    ]
    monkeypatch.setattr(
        agent, "call_llm_with_tools", lambda messages, tools, tool_choice="auto": responses.pop(0)
    )
